...
public class SingularitySchedulerTest extends SingularitySchedulerTestBase {
  ...
  public void testSchedulerIsolatesPendingTasksBasedOnDeploy() {
    ...
    requestManager.addToPendingQueue(new SingularityPendingRequest(requestId, secondDeployId, System.currentTimeMillis(), Optional.<String>empty(), PendingType.NEW_DEPLOY,
        Optional.<Boolean>empty(), Optional.<String>empty()));
  ...
  public void testCleanerLeavesPausedRequestTasksByDemand() {
    ...
    requestResource.pause(requestId, Optional.of(new SingularityPauseRequest(Optional.of(false), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty())), singularityUser);
  ...
  public void testTaskKill() {
    ...
    taskResource.killTask(firstTask.getTaskId().getId(), Optional.empty(), singularityUser);
  ...
  public void testTaskDestroy() {
    ...
    taskResource.killTask(secondTask.getTaskId().getId(), Optional.of(
        new SingularityKillTaskRequest(Optional.of(true), Optional.of("kill -9 bb"), Optional.empty(), Optional.empty(), Optional.empty())), singularityUser);
  ...
  public void testTaskBounce() {
    ...
    taskResource.killTask(firstTask.getTaskId().getId(), Optional.of(
        new SingularityKillTaskRequest(Optional.empty(), Optional.of("msg"), Optional.empty(), Optional.of(true), Optional.empty())), singularityUser);
  ...
  public void testLongRunningTaskKills() {
    ...
    requestManager.activate(request.toBuilder().setKillOldNonLongRunningTasksAfterMillis(Optional.<Long>of(0L)).build(), RequestHistoryType.CREATED, System.currentTimeMillis(), Optional.<String>empty(), Optional.<String>empty());
  ...
  public void testSchedulerExhaustsOffers() {
    ...
    sms.resourceOffers(Arrays.asList(createOffer(2, 1024, 2048), createOffer(1, 1024, 2048))).join();
  ...
  public void testSchedulerRandomizesOffers() {
    ...
    sms.resourceOffers(Arrays.asList(createOffer(20, 1024, 20000), createOffer(20, 1024, 20000))).join();
  ...
  public void testSchedulerHandlesFinishedTasks() {
    ...
    Assertions.assertTrue(requestResource.getActiveRequests(singularityUser, false, false, false, 10, Collections.emptyList()).isEmpty());
    Assertions.assertTrue(requestManager.getRequest(requestId).get().getState() == RequestState.FINISHED);
    Assertions.assertTrue(taskManager.getPendingTaskIds().isEmpty());
  ...
  private void validateTaskDoesntMoveDuringDecommission() {
    scheduler.drainPendingQueue();
    sms.resourceOffers(Arrays.asList(createOffer(1, 129, 1025, "slave1", "host1", Optional.of("rack1")))).join();
    scheduler.drainPendingQueue();
    sms.resourceOffers(Arrays.asList(createOffer(1, 129, 1025, "slave2", "host2", Optional.of("rack1")))).join();

    Assertions.assertEquals(1, taskManager.getActiveTaskIds().size());

    Assertions.assertEquals("host1", taskManager.getActiveTaskIds().get(0).getSanitizedHost());

    Assertions.assertEquals(StateChangeResult.SUCCESS, slaveManager.changeState("slave1", MachineState.STARTING_DECOMMISSION, Optional.<String>empty(), Optional.of("user1")));
  ...
  public void testCustomResourcesWithRunNowRequest() {
    ...
    Assertions.assertEquals(pendingTaskWithResourcs.getResources().get().getCpus(), 2, 0.0);

    sms.resourceOffers(Arrays.asList(createOffer(5, 5, 5, "slave1", "host1", Optional.of("rack1")))).join();

    SingularityTask task = taskManager.getActiveTasks().get(0);
    Assertions.assertEquals(MesosUtils.getNumCpus(mesosProtosUtils.toResourceList(task.getMesosTask().getResources()), Optional.<String>empty()), 2.0, 0.0);
  }
  ...
  public void testRunOnceRunOnlyOnce() {
    ...
    deployResource.deploy(new SingularityDeployRequest(new SingularityDeployBuilder(requestId, "d1").setCommand(Optional.of("cmd")).build(), Optional.empty(), Optional.empty()), singularityUser);
    ...
    Assertions.assertTrue(deployManager.getRequestDeployState(requestId).get().getActiveDeploy().isPresent());
    Assertions.assertTrue(!deployManager.getRequestDeployState(requestId).get().getPendingDeploy().isPresent());
    ...
    deployResource.deploy(new SingularityDeployRequest(new SingularityDeployBuilder(requestId, "d2").setCommand(Optional.of("cmd")).build(), Optional.empty(), Optional.empty()), singularityUser);
  ...
  public void testRetriesWithOverrides() {
    ...
    Assertions.assertEquals(Optional.of(Collections.singletonList("extraFlag")), taskManager.getActiveTasks().get(0).getTaskRequest().getPendingTask().getCmdLineArgsList());
  ...
  public void testLBCleanup() {
    ...
    Assertions.assertTrue(lbUpdate.get().getLoadBalancerState() == BaragonRequestState.WAITING);
    ...
    Assertions.assertTrue(lbUpdate.isPresent());
    Assertions.assertTrue(lbUpdate.get().getLoadBalancerState() == BaragonRequestState.FAILED);
    ...
    Assertions.assertTrue(lbUpdate.isPresent());
    Assertions.assertTrue(lbUpdate.get().getLoadBalancerState() == BaragonRequestState.SUCCESS);
    Assertions.assertTrue(lbUpdate.get().getLoadBalancerRequestId().getAttemptNumber() == 2);
  }
  ...
  public void testLbCleanupDoesNotRemoveBeforeAdd() {
    ...
    Assertions.assertFalse(taskManager.getLoadBalancerState(taskOne.getTaskId(), LoadBalancerRequestType.ADD).isPresent());
    Assertions.assertFalse(taskManager.getLoadBalancerState(taskOne.getTaskId(), LoadBalancerRequestType.REMOVE).isPresent());

    // Second task should have an add request
    Assertions.assertTrue(taskManager.getLoadBalancerState(taskTwo.getTaskId(), LoadBalancerRequestType.ADD).isPresent());
  ...
  public void testLbCleanupSkippedOnSkipRemoveFlag() {
    ...
    SingularityDeleteRequestRequest deleteRequest = new SingularityDeleteRequestRequest(Optional.empty(), Optional.empty(), Optional.of(removeFromLoadBalancer));

    requestResource.deleteRequest(requestId, Optional.of(deleteRequest), singularityUser);

    testingLbClient.setNextBaragonRequestState(BaragonRequestState.WAITING);

    Assertions.assertFalse(requestManager.getCleanupRequests().isEmpty(), "Tasks should get cleaned up");
    cleaner.drainCleanupQueue();
    killKilledTasks();

    Assertions.assertFalse(requestManager.getCleanupRequests().isEmpty(), "The request should get cleaned up");
    cleaner.drainCleanupQueue();

    Assertions.assertTrue(requestManager.getLbCleanupRequestIds().isEmpty(), "The request should not be removed from the load balancer");
  }
  ...
  public void testLbCleanupOccursOnRequestDelete() {
    ...
    Assertions.assertFalse(requestManager.getLbCleanupRequestIds().isEmpty(), "The request should get removed from the load balancer");
  }
  ...
  public void testReconciliation() {
    ...
    Assertions.assertTrue(taskReconciliation.startReconciliation() == ReconciliationState.STARTED);
    Assertions.assertTrue(taskReconciliation.startReconciliation() == ReconciliationState.ALREADY_RUNNING);
  ...
  public void testSchedulerPriority() {
    ...
    Assertions.assertEquals(sortedRequestsByPriority.get(0).getRequest().getId(), highPriorityRequest.getId());
    Assertions.assertEquals(sortedRequestsByPriority.get(1).getRequest().getId(), mediumPriorityRequest.getId());
    Assertions.assertEquals(sortedRequestsByPriority.get(2).getRequest().getId(), lowPriorityRequest.getId());
    ...
    Assertions.assertEquals(sortedRequestsByOverdueAndPriority.get(0).getRequest().getId(), lowPriorityRequest.getId());
    Assertions.assertEquals(sortedRequestsByOverdueAndPriority.get(1).getRequest().getId(), mediumPriorityRequest.getId());
    Assertions.assertEquals(sortedRequestsByOverdueAndPriority.get(2).getRequest().getId(), highPriorityRequest.getId());
  }
  ...
  public void badPauseExpires() {
    ...
    requestManager.createCleanupRequest(new SingularityRequestCleanup(Optional.<String>empty(), RequestCleanupType.PAUSING, System.currentTimeMillis(),
        Optional.<Boolean>empty(), Optional.empty(), requestId, Optional.<String>empty(), Optional.<Boolean>empty(), Optional.<String>empty(), Optional.<String>empty(), Optional.<SingularityShellCommand>empty()));
  ...
  public void testPauseLbCleanup() {
    ...
    requestManager.saveLbCleanupRequest(new SingularityRequestLbCleanup(requestId, Sets.newHashSet("test"), "/basepath", Collections.<String>emptyList(), Optional.<SingularityLoadBalancerUpdate>empty()));
  ...
  public void testBounceOnPendingInstancesReleasesLock() {
    ...
    Assertions.assertEquals(0, taskManager.getActiveTaskIds().size(), "Bounce starts when tasks have not yet been launched");

    requestResource.bounce(requestId, Optional.of(new SingularityBounceRequest(Optional.empty(), Optional.of(true), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty())), singularityUser);

    // It acquires a lock on the bounce
    Assertions.assertTrue(requestManager.getExpiringBounce(requestId).isPresent(), "Lock on bounce should be acquired during bounce");
    ...
    for (SingularityTaskId singularityTaskId : taskManager.getActiveTaskIds()) {
      taskManager.saveTaskHistoryUpdate(new SingularityTaskHistoryUpdate(singularityTaskId, System.currentTimeMillis(), ExtendedTaskState.TASK_RUNNING, Optional.empty(), Optional.empty(), Collections.emptySet()));
    }
    ...
    Assertions.assertEquals(1, taskManager.getActiveTaskIds().size(), "Should end bounce with target number of tasks");
    for (SingularityTaskId singularityTaskId : taskManager.getActiveTaskIds()) {
      String statusMessage = taskManager.getTaskHistoryUpdates(singularityTaskId)
          .get(0)
          .getStatusMessage()
          .get();
      Assertions.assertTrue(statusMessage.contains("BOUNCE"), "Task was started by bounce");
    }
    Assertions.assertFalse(requestManager.getExpiringBounce(requestId).isPresent(), "Lock on bounce should be released after bounce");
  }
  ...
  public void testBounceOnRunningInstancesReleasesLock() {
    ...
    Assertions.assertTrue(taskManager.getActiveTaskIds().size() >= 2, "Need to start at least 1 instance to begin killing old instances");
    Assertions.assertTrue(requestManager.isBouncing(requestId));
    cleaner.drainCleanupQueue();
    killKilledTasks();
  ...
  public void testBounceReleasesLockWithAlternateCleanupType() {
    ...
    taskManager.saveTaskCleanup(new SingularityTaskCleanup(Optional.empty(), TaskCleanupType.USER_REQUESTED, System.currentTimeMillis(), firstTaskId, Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty()));
  ...
  public void testIncrementalBounce() {
    ...
    requestManager.createCleanupRequest(new SingularityRequestCleanup(user, RequestCleanupType.INCREMENTAL_BOUNCE, System.currentTimeMillis(),
        Optional.<Boolean>empty(), Optional.empty(), requestId, Optional.of(firstDeployId), Optional.<Boolean>empty(), Optional.<String>empty(), Optional.<String>empty(), Optional.<SingularityShellCommand>empty()));
  ...
  public void testScheduledNotification() {
    ...
    Assertions.assertTrue(deployStatistics.get().getAverageRuntimeMillis().get() > 1 && deployStatistics.get().getAverageRuntimeMillis().get() < TimeUnit.DAYS.toMillis(1));
    ...
    Assertions.assertTrue(deployStatistics.get().getAverageRuntimeMillis().get() > 1 && deployStatistics.get().getAverageRuntimeMillis().get() < oldAvg);
  ...
  public void testJobRescheduledWhenItFinishesDuringDecommission() {
    ...
    slaveManager.changeState("slave1", MachineState.STARTING_DECOMMISSION, Optional.<String>empty(), Optional.of("user1"));
  ...
  public void testScaleDownTakesHighestInstances() {
    ...
    requestResource.scale(requestId, new SingularityScaleRequest(Optional.of(2), Optional.empty(), Optional.empty(),
        Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty()), singularityUser);
  ...
  public void testScaleDownTakesHighestInstancesWithPendingTask() {
    ...
    requestResource.scale(requestId, new SingularityScaleRequest(Optional.of(3), Optional.empty(), Optional.empty(),
        Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty()), singularityUser);
    scheduler.drainPendingQueue();
    cleaner.drainCleanupQueue();
  ...
  public void testRequestsInPendingQueueAreOrderedByTimestamp() {
    ...
    startFirstDeploy();
    SingularityPendingRequest pendingDeployRequest = new SingularityPendingRequest(requestId, firstDeploy.getId(), now, Optional.empty(), PendingType.NEW_DEPLOY,
        firstDeploy.getSkipHealthchecksOnDeploy(), Optional.empty());
    SingularityPendingRequest pendingRunNowRequest = new SingularityPendingRequest(requestId, firstDeploy.getId(), now + 200, Optional.empty(), PendingType.IMMEDIATE,
        firstDeploy.getSkipHealthchecksOnDeploy(), Optional.empty());
    ...
    // Was added first
    Assertions.assertEquals(PendingType.NEW_DEPLOY, requestManager.getPendingRequests().get(0).getPendingType());
    // Was added second
    Assertions.assertEquals(PendingType.IMMEDIATE, requestManager.getPendingRequests().get(1).getPendingType());
  ...
  public void testWaitAfterTaskWorks() {
    ...
    Assertions.assertTrue(taskManager.getPendingTaskIds().get(0).getNextRunAt() - System.currentTimeMillis() < 1000L);
  ...
  public void testRemovedRequestData() {
    ...
    deployChecker.checkDeploys();
    Assertions.assertEquals(DeployState.WAITING, deployManager.getPendingDeploys().get(0).getCurrentDeployState());

    requestManager.startDeletingRequest(request, Optional.empty(), Optional.<String>empty(), Optional.<String>empty(), Optional.<String>empty());
    requestManager.markDeleted(request, now, Optional.<String>empty(), Optional.<String>empty());
    deployChecker.checkDeploys();
  ...
  public void itCorrectlyUpdatesRequestDeletingStateHistory() {
    ...
    requestManager.startDeletingRequest(request, Optional.empty(), Optional.<String>empty(), Optional.<String>empty(), Optional.of("the cake is a lie"));
    Assertions.assertEquals(RequestState.DELETING, requestManager.getRequest(requestId).get().getState());
  ...
  public void itSetsRequestStateToDeletedAfterAllTasksAreCleanedUp() {
    ...
    Assertions.assertEquals(requestId, requestManager.getActiveRequests().iterator().next().getRequest().getId());
    Assertions.assertEquals(2, taskManager.getActiveTaskIds().size());

    requestManager.startDeletingRequest(request, Optional.empty(), Optional.empty(), Optional.empty(), Optional.empty());

    Assertions.assertEquals(requestId, requestManager.getCleanupRequests().get(0).getRequestId());
    Assertions.assertEquals(RequestState.DELETING, requestManager.getRequest(requestId).get().getState());
  ...
  public void testMaxTasksPerOffer() {
    ...
    sms.resourceOffers(Arrays.asList(createOffer(20, 20000, 50000, "slave1", "host1"), createOffer(20, 20000, 50000, "slave2", "host2"))).join();
  ...
  public void testRequestedPorts() {
    ...
    sms.resourceOffers(Arrays.asList(createOffer(20, 20000, 50000, "slave1", "host1", Optional.<String>empty(), Collections.<String, String>emptyMap(), portRangeWithNoRequestedPorts))).join();
    Assertions.assertEquals(0, taskManager.getActiveTasks().size());

    String[] portRangeWithSomeRequestedPorts = {"80:82"};
    sms.resourceOffers(Arrays.asList(createOffer(20, 20000, 50000, "slave1", "host1", Optional.<String>empty(), Collections.<String, String>emptyMap(), portRangeWithSomeRequestedPorts))).join();
    Assertions.assertEquals(0, taskManager.getActiveTasks().size());

    String[] portRangeWithRequestedButNotEnoughPorts = {"80:80", "8080:8080"};
    sms.resourceOffers(Arrays.asList(createOffer(20, 20000, 50000, "slave1", "host1", Optional.<String>empty(), Collections.<String, String>emptyMap(), portRangeWithRequestedButNotEnoughPorts))).join();
    Assertions.assertEquals(0, taskManager.getActiveTasks().size());

    String[] portRangeWithNeededPorts = {"80:83", "8080:8080"};
    sms.resourceOffers(Arrays.asList(createOffer(20, 20000, 50000, "slave1", "host1", Optional.<String>empty(), Collections.<String, String>emptyMap(), portRangeWithNeededPorts))).join();
    Assertions.assertEquals(1, taskManager.getActiveTaskIds().size());
  }
  ...
  public void testQueueMultipleOneOffs() {
    ...
    requestManager.addToPendingQueue(new SingularityPendingRequest(requestId, "on_demand_deploy", System.currentTimeMillis(), Optional.<String>empty(), PendingType.ONEOFF,
      Optional.<List<String>>empty(), Optional.<String>empty(), Optional.<Boolean>empty(), Optional.<String>empty(), Optional.<String>empty()));
    requestManager.addToPendingQueue(new SingularityPendingRequest(requestId, "on_demand_deploy", System.currentTimeMillis(), Optional.<String>empty(), PendingType.ONEOFF,
      Optional.<List<String>>empty(), Optional.<String>empty(), Optional.<Boolean>empty(), Optional.<String>empty(), Optional.<String>empty()));
  ...
  public void testPriorityFreezeKillsActiveTasks() {
    ...
    priorityResource.createPriorityFreeze(singularityUser, new SingularityPriorityFreeze(.5, true, Optional.of("test"), Optional.empty()));
    ...
    Assertions.assertEquals(TaskCleanupType.PRIORITY_KILL, taskManager.getTaskCleanup(lowPriorityTask.getTaskId().getId()).get().getCleanupType());

    Assertions.assertEquals(false, taskManager.getTaskCleanup(mediumPriorityTask.getTaskId().getId()).isPresent());
    Assertions.assertEquals(false, taskManager.getTaskCleanup(highPriorityTask.getTaskId().getId()).isPresent());
    ...
    Assertions.assertEquals(ExtendedTaskState.TASK_KILLED, taskManager.getTaskHistory(lowPriorityTask.getTaskId()).get().getLastTaskUpdate().get().getTaskState());
    Assertions.assertEquals(ExtendedTaskState.TASK_RUNNING, taskManager.getTaskHistory(mediumPriorityTask.getTaskId()).get().getLastTaskUpdate().get().getTaskState());
    Assertions.assertEquals(ExtendedTaskState.TASK_RUNNING, taskManager.getTaskHistory(highPriorityTask.getTaskId()).get().getLastTaskUpdate().get().getTaskState());
    ...
    Assertions.assertNotEquals(lowPriorityTask.getTaskId(), taskManager.getActiveTaskIdsForRequest(lowPriorityRequest.getId()).get(0).getId());
  }
  ...
  public void testPriorityFreezeDoesntLaunchTasks() {
    ...
    deployResource.deploy(
        new SingularityDeployRequest(new SingularityDeployBuilder(lowPriorityRequest.getId(), "d1").setCommand(Optional.of("cmd")).build(), Optional.empty(), Optional.empty()), singularityUser);
    ...
    saveRequest(mediumPriorityRequest);
    deployResource.deploy(
        new SingularityDeployRequest(new SingularityDeployBuilder(mediumPriorityRequest.getId(), "d2").setCommand(Optional.of("cmd")).build(), Optional.empty(), Optional.empty()), singularityUser);

    // create priority freeze
    priorityManager.createPriorityFreeze(
        new SingularityPriorityFreezeParent(new SingularityPriorityFreeze(0.3, true, Optional.<String>empty(), Optional.<String>empty()), System.currentTimeMillis(), Optional.<String>empty()));
    ...
    Assertions.assertEquals(1, taskManager.getPendingTaskIds().size());
    Assertions.assertEquals(lowPriorityRequest.getId(), taskManager.getPendingTaskIds().get(0).getRequestId());

    // assert that only mediumPriorityRequest has an active task
    Assertions.assertEquals(0, taskManager.getActiveTaskIdsForRequest(lowPriorityRequest.getId()).size());
    Assertions.assertEquals(1, taskManager.getActiveTaskIdsForRequest(mediumPriorityRequest.getId()).size());

    // delete priority freeze
    Assertions.assertEquals(SingularityDeleteResult.DELETED, priorityManager.deleteActivePriorityFreeze());
    ...
    Assertions.assertEquals(1, taskManager.getActiveTaskIdsForRequest(lowPriorityRequest.getId()).size());
    Assertions.assertEquals(1, taskManager.getActiveTaskIdsForRequest(mediumPriorityRequest.getId()).size());
  }
  ...
  public void testCronScheduleChanges() throws Exception {
    ...
    request = validator.checkSingularityRequest(request, Optional.<SingularityRequest>empty(), Optional.<SingularityDeploy>empty(), Optional.<SingularityDeploy>empty());

    saveRequest(request);

    Assertions.assertEquals(oldScheduleQuartz, requestManager.getRequest(requestId).get().getRequest().getQuartzScheduleSafe());
    ...
    deployResource.deploy(new SingularityDeployRequest(newDeploy, Optional.empty(), Optional.empty(), Optional.of(newRequest)), singularityUser);

    deployChecker.checkDeploys();

    scheduler.drainPendingQueue();

    Assertions.assertEquals(newScheduleQuartz, requestManager.getRequest(requestId).get().getRequest().getQuartzScheduleSafe());
  }
  ...
  public void testImmediateRunReplacesScheduledTask() {
    ...
    requestManager.addToPendingQueue(new SingularityPendingRequest(requestId, deploy.getId(), System.currentTimeMillis(), Optional.empty(), PendingType.IMMEDIATE,
        deploy.getSkipHealthchecksOnDeploy(), Optional.empty()));
    scheduler.drainPendingQueue();
  ...
  public void testSchedulerDropsMultipleScheduledTaskInstances() {
    ...
    requestManager.addToPendingQueue(new SingularityPendingRequest(requestId, firstDeployId,
        ...
        Optional.empty(), PendingType.NEW_DEPLOY, Optional.empty(), Optional.empty()));
  ...
  public void testInvalidQuartzTimeZoneErrors() {
    ...
    Assertions.assertThrows(WebApplicationException.class, () -> requestResource.postRequest(req, singularityUser));
  }
  ...
  public void testDifferentQuartzTimeZones() {
    ...
    deployResource.deploy(new SingularityDeployRequest(deployEST, Optional.empty(), Optional.empty(), Optional.empty()), singularityUser);
    deployResource.deploy(new SingularityDeployRequest(deployGMT, Optional.empty(), Optional.empty(), Optional.empty()), singularityUser);
  ...
  public void testDeployCleanupOverwritesTaskBounceCleanup() {
    ...
    taskResource
      .killTask(oldTask.getTaskId().getId(), Optional.of(new SingularityKillTaskRequest(Optional.empty(), Optional.empty(), Optional.empty(), Optional.of(true), Optional.empty())), singularityUser);
    ...
    Assertions.assertEquals(TaskCleanupType.USER_REQUESTED_TASK_BOUNCE, taskCleanup.get().getCleanupType());
    ...
    Assertions.assertEquals(TaskCleanupType.DEPLOY_STEP_FINISHED, taskManager.getTaskCleanup(oldTask.getTaskId().getId()).get().getCleanupType());

    cleaner.drainCleanupQueue();

    Assertions.assertFalse(taskManager.getTaskCleanup(oldTask.getTaskId().getId()).isPresent());
  }
  ...
  public void testCleanerFindsTasksWithSkippedHealthchecks() {
    ...
    requestManager.saveHistory(new SingularityRequestHistory(now, Optional.<String>empty(), RequestHistoryType.UPDATED,
      request.toBuilder()
    ...
    requestManager.saveHistory(new SingularityRequestHistory(now + 3000, Optional.<String>empty(), RequestHistoryType.UPDATED,
      updatedRequest, Optional.<String>empty()));
    ...
    Assertions.assertEquals(DeployHealth.WAITING, deployHealthHelper.getDeployHealth(updatedRequest, Optional.of(firstDeploy), activeTaskIds, false));
    ...
    Assertions.assertEquals(DeployHealth.HEALTHY, deployHealthHelper.getDeployHealth(updatedRequest, Optional.of(firstDeploy), activeTaskIds, false));
  }
  ...
  public void testScaleWithBounceDoesNotLaunchExtraInstances() {
    ...
    requestResource.scale(requestId, new SingularityScaleRequest(Optional.of(5), Optional.of(1L), Optional.empty(), Optional.empty(), Optional.empty(), Optional.of(true), Optional.empty(), Optional.empty()), singularityUser);
  ...
  public void testAcceptOffersWithRoleForRequestWithRole() {
    ...
    Assertions.assertEquals(pendingTaskWithResources.getResources().get().getCpus(), 2, 0.0);
    ...
    sms.resourceOffers(Arrays.asList(createOffer(5, 5, 5, Optional.of("test-role")))).join();
    SingularityTask task = taskManager.getActiveTasks().get(0);
    Assertions.assertEquals(MesosUtils.getNumCpus(mesosProtosUtils.toResourceList(task.getMesosTask().getResources()), Optional.of("test-role")), 2.0, 0.0);
  }
  ...
  public void testCleanupsCreatedOnScaleDown() {
    ...
    requestResource.postRequest(bldr.build(), singularityUser);
    Assertions.assertEquals(1, taskManager.getNumCleanupTasks());
    Assertions.assertEquals(taskManager.getCleanupTaskIds().get(0), secondTask.getTaskId());
  }
